...
internal sealed unsafe class Dx12Win32Scene : IWin32Scene
{
    ...
    {
        fixed (void* pixelData = data)
        {
            return this.LoadImageRaw(
                pixelData,
                width * numChannels,
                width,
                height,
                numChannels == 4 ? DXGI_FORMAT.DXGI_FORMAT_R8G8B8A8_UNORM : DXGI_FORMAT.DXGI_FORMAT_R8_UNORM);
        }
    }
    ...
    public IDalamudTextureWrap LoadImageFormat(ReadOnlySpan<byte> data, int pitch, int width, int height, int format)
    {
        fixed (void* pData = data)
            return this.LoadImageRaw(pData, pitch, width, height, (DXGI_FORMAT)format);
    }
