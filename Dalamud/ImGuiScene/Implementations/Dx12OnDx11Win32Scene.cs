...
internal unsafe class Dx12OnDx11Win32Scene : IWin32Scene
{
    ...
    private ComPtr<ID3D11Device1> device;
    ...
    public Dx12OnDx11Win32Scene(IDXGISwapChain* swapChain)
    {
        try
        {
            ...
            fixed (ID3D11Device1** pp = &this.device.GetPinnableReference())
            ...
            this.drawsOneSquare.Setup(this.device);
    ...
    /// </summary>
    public ID3D11Device1* Device => this.device;
    ...
    public void Render()
    {
        ...
        {
            HANDLE handle;
            this.device12.Get()->CreateSharedHandle(
                (ID3D12DeviceChild*)this.scene12.Renderer.MainRenderTarget,
                null,
                Win32.GENERIC_ALL,
                null,
                &handle).ThrowHr();
            try
            {
                ...
                fixed (Guid* piid = &IID.IID_ID3D11Texture2D)
                    this.device.Get()->OpenSharedResource1(handle, piid, (void**)tex.GetAddressOf()).ThrowHr();
        ...
        this.drawsOneSquare.Draw(this.deviceContext, this.srv, this.targetWidth, this.targetHeight);
    ...
    /// <inheritdoc/>
    public bool IsImGuiCursor(IntPtr cursorHandle) => this.scene12.IsImGuiCursor(cursorHandle);
    ...
    /// <inheritdoc/>
    public IDalamudTextureWrap LoadImage(string path) => this.scene12.LoadImage(path);

    /// <inheritdoc/>
    public IDalamudTextureWrap LoadImage(ReadOnlySpan<byte> data) => this.scene12.LoadImage(data);

    /// <inheritdoc/>
    public IDalamudTextureWrap LoadImageRaw(ReadOnlySpan<byte> data, int width, int height, int numChannels = 4) =>
        this.scene12.LoadImageRaw(data, width, height, numChannels);

    /// <inheritdoc/>
    public IDalamudTextureWrap LoadImageFormat(ReadOnlySpan<byte> data, int pitch, int width, int height, int format) =>
        this.scene12.LoadImageFormat(data, pitch, width, height, format);
    ...
    public nint CreateTexturePipeline(ReadOnlySpan<byte> ps, in D3D12_STATIC_SAMPLER_DESC samplerDesc) =>
        this.scene12.CreateTexturePipeline(ps, samplerDesc);
    ...
    public void ReleaseTexturePipeline(IntPtr pipelineHandle) =>
        this.scene12.ReleaseTexturePipeline(pipelineHandle);
