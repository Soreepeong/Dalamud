...
/// <summary>A payload that inserts a soft hyphen.</summary>
public sealed class SoftHyphenPayload : IMutableSePayload
{
    /// <summary>The singleton instance of <see cref="SoftHyphenPayload"/>.</summary>
    public static readonly SoftHyphenPayload Instance = new();

    private SoftHyphenPayload()
    {
    }

    /// <summary>Gets the immutable bytes for representing this payload.</summary>
    public static ReadOnlySpan<byte> Bytes => "\x02\x16\x01\x03"u8;
    ...
    /// <inheritdoc/>
    public int MacroCode => (int)Lumina.Text.Payloads.MacroCode.SoftHyphen;
