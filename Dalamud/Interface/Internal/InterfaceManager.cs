...
internal class InterfaceManager : IDisposable, IServiceType
{
    ...
    private unsafe delegate IntPtr PresentDelegate(IDXGISwapChain* swapChain, uint syncInterval, uint presentFlags);

    [UnmanagedFunctionPointer(CallingConvention.ThisCall)]
    private unsafe delegate IntPtr ResizeBuffersDelegate(IDXGISwapChain* swapChain, uint bufferCount, uint width, uint height, uint newFormat, uint swapChainFlags);

    [UnmanagedFunctionPointer(CallingConvention.StdCall)]
    private delegate HCURSOR SetCursorDelegate(HCURSOR hCursor);

    [UnmanagedFunctionPointer(CallingConvention.StdCall)]
    private unsafe delegate IntPtr DispatchMessageWDelegate(MSG* msg);
    ...
    /// <inheritdoc cref="IImGuiScene.LoadImage(string)"/>
    public IDalamudTextureWrap LoadImage(string filePath) =>
        new DalamudTextureWrap(
            this.scene?.LoadImage(filePath) ?? throw new InvalidOperationException("Scene isn't ready."));

    /// <inheritdoc cref="IImGuiScene.LoadImage(ReadOnlySpan{byte})"/>
    public IDalamudTextureWrap? LoadImage(ReadOnlySpan<byte> imageData) =>
        new DalamudTextureWrap(
            this.scene?.LoadImage(imageData) ?? throw new InvalidOperationException("Scene isn't ready."));

    /// <inheritdoc cref="IImGuiScene.LoadImageRaw"/>
    public IDalamudTextureWrap? LoadImageRaw(ReadOnlySpan<byte> imageData, int width, int height, int numChannels) =>
        new DalamudTextureWrap(
            this.scene?.LoadImageRaw(imageData, width, height, numChannels)
            ?? throw new InvalidOperationException("Scene isn't ready."));

    /// <inheritdoc cref="IImGuiScene.LoadImageFormat"/>
    public IDalamudTextureWrap LoadImageFormat(Span<byte> data, int pitch, int width, int height, int format) =>
        new DalamudTextureWrap(
            this.scene?.LoadImageFormat(data, pitch, width, height, format)
            ?? throw new InvalidOperationException("Scene isn't ready."));
     ...
     */
    private unsafe IntPtr PresentDetour(IDXGISwapChain* swapChain, uint syncInterval, uint presentFlags)
    {
        if (this.scene == null)
    ...
    private unsafe void ContinueConstruction(TargetSigScanner sigScanner, Framework framework)
    {
        this.address.Setup(sigScanner);
        framework.RunOnFrameworkThread(() =>
        {
            fixed (void* ffxivgame = "F\0F\0X\0I\0V\0G\0A\0M\0E\0\0\0"u8)
            {
    ...
    private unsafe IntPtr DispatchMessageWDetour(MSG* msg)
    {
        if (msg->hwnd == this.GameWindowHandle && this.scene != null)
        {
            var res = this.scene.ProcessWndProcW(msg->hwnd, msg->message, msg->wParam, msg->lParam);
            if (res != null)
                return res.Value;
        }
        ...
        return this.dispatchMessageWHook.IsDisposed
                   ...
                   : this.dispatchMessageWHook.Original(msg);
    }

    private unsafe IntPtr ResizeBuffersDetour(IDXGISwapChain* swapChain, uint bufferCount, uint width, uint height, uint newFormat, uint swapChainFlags)
    {
#if DEBUG
